...
/** @brief Parse mapping button

    @param[in] document The yaml document to use
    @param[in] index The node index for mapping value (scalar str tag)
    @param[in] button The button to map
    @param[in] controller The controller button names hint

    @returns
        This function returns -1 on error and 0 on success
 */
int hori_yaml_parse_profile_button_mapping(yaml_document_t* document, int index, hori_profile_t* profile, int button, int controller) {
    if (!document || !profile || button < 0)
        return -1;
    int value = button;
    const yaml_node_t* node = yaml_document_get_node(document, index);
    if (node) {
        if (!hori_yaml_is_node(node, YAML_SCALAR_NODE, YAML_STR_TAG))
            return -1;
        value = hori_get_button_value(node->data.scalar.value, controller);
        if (value < 0)
            return -1;
    }
    if (-1 == hori_set_profile_button(profile, button, HORI_PROFILE_BUTTON_MAPPING_ENABLED, value != button))
        return -1;
    if (-1 == hori_set_profile_button(profile, button, HORI_PROFILE_BUTTON_MAPPING_VALUE, value))
        return -1;
    return 0;
}


int hori_yaml_parse_profile_button_outer_deadzone(yaml_document_t* document, int index, hori_profile_t* profile, int button) {
    if (!document || !profile || button < 0)
        return -1;
    int value = button;
    const yaml_node_t* node = yaml_document_get_node(document, index);
...
int hori_yaml_parse_profile_button_inner_deadzone(yaml_document_t* document, int index, hori_profile_t* profile, int button) {
    if (!document || !profile || button < 0)
        return -1;
    int value = button;
    const yaml_node_t* node = yaml_document_get_node(document, index);
...
/** @brief Parse button curve
    @returns
        This function returns -1 on error
 */
int hori_yaml_parse_profile_button_curve(yaml_document_t* document, int index, hori_profile_t* profile, int button) {
    if (!document || !profile || button < 0)
        return -1;
    union {
        struct {
    ...
    const int props[2][2] = {
        { HORI_PROFILE_BUTTON_ANALOG_MOVEMENT_A_VALUE, HORI_PROFILE_BUTTON_ANALOG_RESPONSE_A_VALUE },
        { HORI_PROFILE_BUTTON_ANALOG_MOVEMENT_A_VALUE, HORI_PROFILE_BUTTON_ANALOG_RESPONSE_A_VALUE },
    };
    int enabled = 0;
    for (int point = 0; point < 2; ++point)
        for (int i = 0; i < 2; ++i)
            if (-1 == hori_set_profile_button(profile, button, props[point][i], curve[point].raw[i]))
                return -1;
            else
                enabled += curve[point].raw[i];
    return hori_set_profile_button(profile, button, HORI_PROFILE_BUTTON_ANALOG_LINEAR_ENABLED, enabled != 0);
}
...
int hori_yaml_parse_profile_button(yaml_document_t* document, int index, hori_profile_t* profile, int controller, int button) {
    ...
    if (-1 == hori_yaml_parse_profile_button_turbo(document, data.turbo, profile, button))
        return -1;
    ...
    if (-1 == hori_yaml_parse_profile_button_inner_deadzone(document, data.inner_deadzone, profile, button))
        return -1;
    if (-1 == hori_yaml_parse_profile_button_outer_deadzone(document, data.outer_deadzone, profile, button))
        return -1;
    return 0;
}
